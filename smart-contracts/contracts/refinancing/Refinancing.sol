    ...
    ReentrancyGuard
{
    ...
    function refinanceLoan(
        ...
        LoanData.CollectionIdRange memory idRange;
        _refinance(targetLoanType, _refinancingData, _offer, idRange, _lenderSignature, _extraData);
    }
    ...
    function refinanceCollectionRangeOfferLoan(
        ...
        _refinance(targetLoanType, _refinancingData, _offer, _idRange, _lenderSignature, _extraData);
    }
    ...
    function _refinanceFlashloanCallback(
        ...
        // check collateral,
        // if this contract owns it, we are OK
        // if borrower owns it, transfer (non transferrable borrower role case)
        // if neither this contract, nor borrower owns it, error
        if (!_isNFTOwner(collateralContract, collateralId, address(this))) {
            if (!_isNFTOwner(collateralContract, collateralId, _borrower)) {
                revert collateralNotOwned();
            } else {
                _transferNFT(collateralContract, collateralId, _borrower, address(this));
            }
        }
    ...
    function _transferNFT(
        ...
        Address.functionDelegateCall(
            nftCollateralWrapper,
            abi.encodeWithSelector(
                INftWrapper(nftCollateralWrapper).transferNFT.selector,
                _sender,
                _recipient,
                _nftCollateralContract,
                _nftCollateralId
